...
namespace NEM.Views
{
    public sealed partial class Player : Page, IPageWithViewModel<PlayerViewModel>, IFullscreenPage
    {
        ...
        private void StartInking(NowPlayingState nowPlaying, byte[] imageBytes)
        {
            ...
            {
                NEM.DataModel.InkNote meme = new NEM.DataModel.InkNote(nowPlaying.CurrentEpisode.Key, PlayerService.Current.CurrentTime);
        ...
        {
            if (e.Parameter is NEM.DataModel.InkNote)
            {
                var paramItem = e.Parameter as NEM.DataModel.InkNote;
